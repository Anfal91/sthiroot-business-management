...
export function MyOffers() {
  ...
  const userId = '0502241234'; // Amit Kumar's ID
  
  const userProgress = getUserOfferProgress(userId);
  const activeOffers = getActiveOffers('partner');
  const pastOffers = getExpiredOffers('partner');
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                <Gift className={`h-5 w-5 ${isCompleted ? 'text-green-600' : 'text-emerald-600'}`} />
            ...
            <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="text-xs text-yellow-800">
                  ...
                  <p>You haven't started working towards this offer yet.</p>
                </div>
              </div>
            </div>
          )}
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">Track your progress on available offers and rewards</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white rounded-lg shadow-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-emerald-100 text-sm">Active Offers</p>
              <p className="text-3xl font-bold mt-1">{activeOfferIds.length}</p>
            </div>
            <Tag className="h-10 w-10 text-emerald-100" />
          </div>
        </div>
        ...
        {activeTab === 'active' ? (
          activeOfferIds.length === 0 ? (
            <div className="col-span-full bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
              ...
              <p className="text-gray-600">Check back later for new opportunities and rewards</p>
            </div>
          ) : (
