...
import { Wallet, ArrowUpCircle, ArrowDownCircle, AlertTriangle, Eye, History, Smartphone, QrCode, Building2, Copy } from 'lucide-react';
...
const PARTNER_ID = 'PART-001'; // Current logged-in partner
...
export function PartnerWallet() {
  ...
  const partnerWallet = mockWallets.find(w => w.userId === PARTNER_ID);
  
  // Get partner's transactions
  const partnerTransactions = mockWalletTransactions.filter(t => t.userId === PARTNER_ID);
  
  // Get partner's withdraw requests
  const partnerWithdrawRequests = mockWithdrawRequests.filter(r => r.userId === PARTNER_ID);
  
  // Get partner's top-up requests
  const partnerTopUpRequests = mockTopUpRequests.filter(r => r.userId === PARTNER_ID);
  
  // Get partner's reports
  const partnerReports = mockReportedTransactions.filter(r => r.userId === PARTNER_ID);
  ...
  const handleWithdrawRequest = (e: React.FormEvent) => {
    ...
    if (parseFloat(withdrawForm.amount) > (partnerWallet?.balance || 0)) {
              ...
              <form onSubmit={handleWithdrawRequest} className="space-y-4">
                <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
                  ...
                  <p className="text-2xl font-bold text-emerald-600">
                    ₹{partnerWallet?.balance.toLocaleString() || '0'}
                  </p>
                </div>
          ...
          <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-emerald-200">
            <div>
              ...
              <p className="text-lg font-semibold text-green-600">
                ₹{partnerWallet?.totalReceived.toLocaleString() || '0'}
              </p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Total Withdrawn</p>
              <p className="text-lg font-semibold text-orange-600">
                ₹{partnerWallet?.totalWithdraw.toLocaleString() || '0'}
              </p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Status</p>
              <Badge className="mt-1">{partnerWallet?.status || 'Active'}</Badge>
            </div>
          </div>
        </CardContent>
      </Card>
                ...
                <TableBody>
                  {partnerTransactions.length > 0 ? (
                    partnerTransactions.map((txn) => {
                      const isCredit = ['TopUp', 'Received', 'Refund', 'Commission', 'Reward'].includes(txn.type);
                      return (
                      <TableRow key={txn.id}>
                        ...
                        <TableCell className={isCredit ? 'text-green-600 font-semibold' : 'text-orange-600 font-semibold'}>
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                setReportingTransaction(txn);
                                setReportForm({
                                  ...reportForm,
                                  transactionId: txn.transactionNumber,
                                  expectedAmount: txn.amount.toString(),
                                });
                                setOpenReport(true);
                              }}
                            >
          ...
          <Card>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {partnerTopUpRequests.length > 0 ? (
                    partnerTopUpRequests.map((req) => (
                      <TableRow key={req.id}>
                        ...
                        <TableCell className="font-mono text-sm">{req.transactionId}</TableCell>
                        <TableCell>
                          <Badge
                            variant={
        ...
        <TabsContent value="withdrawals">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Track your wallet withdrawal requests</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {partnerWithdrawRequests.length > 0 ? (
                    partnerWithdrawRequests.map((req) => (
                      <TableRow key={req.id}>
                        <TableCell className="font-mono text-sm">{req.requestNumber}</TableCell>
                        <TableCell>{new Date(req.requestedAt).toLocaleDateString()}</TableCell>
                        <TableCell className="font-semibold">₹{req.requestedAmount.toLocaleString()}</TableCell>
                        <TableCell>
                          <Badge variant="outline">{req.upiId ? 'UPI' : 'BANK'}</Badge>
                        </TableCell>
                        <TableCell>
                          <Badge
                            variant={
        ...
        <TabsContent value="reports">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Track your reported transaction issues</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  ...
                  {partnerReports.length > 0 ? (
                    partnerReports.map((report) => (
                      <TableRow key={report.id}>
                        ...
                        <TableCell className="font-mono text-sm">{report.transactionId}</TableCell>
                        <TableCell>{report.reason}</TableCell>
                        <TableCell className="font-semibold">₹{report.expectedAmount.toLocaleString()}</TableCell>
                        <TableCell>
                          <Badge
                            variant={
          ...
          <DialogContent>
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="font-mono font-semibold">{selectedTransaction.transactionNumber}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className={`text-2xl font-bold ${selectedTransaction.type === 'credit' ? 'text-green-600' : 'text-orange-600'}`}>
